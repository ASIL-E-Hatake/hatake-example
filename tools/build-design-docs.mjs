...
// ので上げた。案件ごとに違うのは「API の実装がどこか」「定義に書けない口は何か」だけ
// なので、そこだけ案件の `tools/design.config.json` から読む。
//
// 使い方（案件のフォルダで叩く。案件側の `tools/build-design-docs.mjs` はここを呼ぶだけ）:
...
/** 案件ごとに違うところ（無ければ既定でいく）。 */
const CONFIG = existsSync("tools/design.config.json")
  ? JSON.parse(readFileSync("tools/design.config.json", "utf8"))
  : {};
const IMPL = CONFIG.impl ?? "node-src";
...
function kindOf(page) {
  ...
  if (headline.includes("1件の照会")) return "detail";
  if (headline.includes("登録・修正・削除")) return "crud";
  if (headline.includes("ステップ")) return "wizard";
  if (headline.includes("段階入力")) return "wizard";
  if (headline.includes("帳票")) return "report";
  if (headline.includes("数字とグラフ")) return "dashboard";
  if (headline.includes("カード")) return "dashboard";
  if (headline.includes("照会")) return "search";
...
// REST に載せる形も決まっている（`hatake_http` の契約）＝定義から機械的に出せる。
{
  ...
  for (const page of pages) {
    ...
    const base = PATHS[page.repository] ?? `/api/${page.repository.replace(/Repository$/, "")}s`;
  ...
  // 画面が使っていない Repository（選択肢の出どころだけ、など）も口は要る。
  for (const extra of CONFIG.repositories ?? []) {
    if (seen.has(extra.name)) continue;
    seen.add(extra.name);
    const base = PATHS[extra.name] ?? `/api/${extra.name.replace(/Repository$/, "")}s`;
    blocks.push(
      [
        `### \`${extra.name}\` → \`${base}\``,
        "",
        `使う画面: ${extra.用途 ?? extra.what ?? "選択肢の出どころ"}`,
        "",
        "| メソッド | パス | 何をするか |",
        "|---|---|---|",
        ...contract.map(([method, tail, what]) => `| \`${method}\` | \`${base}${tail}\` | ${what} |`),
  ...
  write(
    ...
    [
      ...
      "",
      `実装は [${IMPL}](../../${IMPL}/)。**画面と同じ定義で検証している**ので、`,
      ...
      "|---|---|---|---|",
      ...OUTSIDE.map((one) => `| \`${one.method}\` | \`${one.path}\` | ${one.what} | ${one.why} |`),
      "",
      "## 受け取る形・返す形（スキーマ）",
      "",
